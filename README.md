...
4. Review and customize:
   ...
   - Set each host's `stateVersion` to the release used for its initial NixOS and Home Manager installation
...
```bash
# Format and check the configuration (do not run these as root)
nix fmt .
nix flake check

# Update inputs when desired (also do not run as root)
nix flake update
...
3. Add to `flake.nix`:
   ...
   nixosConfigurations.NEWHOSTNAME = mkNixOS {
     ...
     stateVersion = "26.05";  # release used for initial installation
   };
   ```
